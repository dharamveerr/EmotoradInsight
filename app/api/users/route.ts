...
export async function POST(req: NextRequest) {
  ...
  const { username, email, name, password, role, client_id, permissions } = body;
  ...
  // Store exactly what the super admin chose. Default = no report access
  // (empty array) — set explicitly so the NULL backfill never re-grants it.
  ...
  try {
    await db.prepare(
      `INSERT INTO app_users (id, username, email, name, role, is_active, password_hash, client_id, permissions, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?, ?, ?)`
    ).run(uuidv4(), username || null, email || null, name || username || email, role, hashPassword(password), client_id || null, permsJson, now, now);
